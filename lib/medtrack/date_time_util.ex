defmodule Medtrack.DateTimeUtil do
  require Logger
  ...
  def format_time(datetime, options \\ []) do
    ...
    Logger.debug(
      "Formatting datetime #{inspect(datetime)} with options #{inspect(options)} and format #{inspect(format)}"
    )

    with time_in_tz <- Timex.Timezone.convert(Timex.to_datetime(datetime, "UTC"), timezone),
